    ...
    <div class="content_area">
      <div class="main_content floatleft">
		...
		?>
          ...
          <?php $sql_search_user=mysqli_query($dbconfig,"(SELECT * FROM user 
				...
				if($check_search_user!=''){
  				while($search1=mysqli_fetch_assoc($sql_search_user)){ ?>
                ...
                <?php $sql_search_college=mysqli_query($dbconfig,"(SELECT * FROM colleges 
				WHERE instituteName LIKE '%".$searchVal."%' OR collegeAddress LIKE '%".$searchVal."%')") or die (mysqli_error($dbconfig));
				$check_search_college=mysqli_num_rows($sql_search_college);
				if($check_search_college!=''){
  				while($search1=mysqli_fetch_assoc($sql_search_college)){ ?>
                ...
                <?php $sql_search_coach=mysqli_query($dbconfig,"(SELECT * FROM coaching_institute 
				WHERE instituteName LIKE '%".$searchVal."%' OR ci_Address LIKE '%".$searchVal."%' )") or die (mysqli_error($dbconfig));
				$check_search_coach=mysqli_num_rows($sql_search_coach);
				if($check_search_coach!=''){
  				while($search1=mysqli_fetch_assoc($sql_search_coach)){ ?>
                ...
                <?php $sql_search_event=mysqli_query($dbconfig,"(SELECT * FROM events
				LEFT JOIN user ON events.userID=user.userID
				WHERE eventName LIKE '%".$searchVal."%' OR webName LIKE '%".$searchVal."%')") or die (mysqli_error($dbconfig));
				$check_search_event=mysqli_num_rows($sql_search_event);
				if($check_search_event!=''){
  				while($search1=mysqli_fetch_assoc($sql_search_event)){ ?>
                 <!--Event Result-->
			...
			WHERE uploadDoc LIKE '%".$searchVal."%' OR detail LIKE '%".$searchVal."%' ORDER BY uploadID DESC") or die (mysqli_error($dbconfig));
			$check_uploadDoc=mysqli_num_rows($sqlUpload);
			if($check_uploadDoc !=''){
