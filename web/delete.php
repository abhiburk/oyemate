...
if($_REQUEST['xAction'] == 'deleteFee'){
	...
	{   $coID=$_REQUEST['coordinateID'];
		...
		mysql_query($sql1) or die(mysql_error());
		header('location:myevent.php?eventID='.$eID.''); exit;
	}
	...
	else if($_REQUEST['xAction'] == 'deleteCoachingStud')
	{   $userID=$_REQUEST['userID'];
	    $ci_ID=$_REQUEST['ci_ID'];
		$sql1 = "DELETE FROM coaching_institute_students WHERE userID = '".$userID."'";	
		mysql_query($sql1) or die(mysql_error());
		header('location:mycoaching.php?ci_ID='.$ci_ID.''); exit;
	}
	else{
