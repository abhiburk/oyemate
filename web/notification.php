          ...
          <div class="text_area_home ">
           <?php
            ...
            <?php 
				...
				$notification=mysqli_query($dbconfig,"SELECT * FROM improve_us LEFT JOIN user ON improve_us.userID=user.userID
				WHERE user.userID='".$_SESSION['userid']."' AND replyMessage!=''
				ORDER BY improveID DESC");
				$count_noti=mysqli_num_rows($notification);
				if($count_noti=='') {echo 'No Notificaton Available';}else {
  				while($row_notification=mysqli_fetch_assoc($notification)){ ?>
            <div class="left-menu-margin"  style=" <?php if($row_notification['readStatus']=='unread'){ ?> background-color:#E4E4E4; <?php }?>  padding:5px; margin-bottom:3px;">
              <p style="display:inline; <?php if($row_notification['readStatus']=='unread'){ ?> font-weight:bold<?php }?>"> 
               You have reply from oyemate for your message <b><?php echo $row_notification['improveMessage']; ?></b><a href="viewNotification.php?improveID=<?php echo $row_notification['improveID']; ?>"> See </a> 
              <small class="floatright"><?php echo "" . humanTiming( $row_notification['replyTime'] ). " ago"; ?></small> </p>
            </div>
            <?php }} ?>
          </div><?php  ?>
